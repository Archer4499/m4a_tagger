...
def disambig(source, data, title):
    ...
    if source == 'html':
        for item in lxml.html.fromstring(data).xpath("//p[1]"):
            if "may refer to:" in etree.tostring(item, encoding="unicode"):
    ...
    for item in lxml.html.fromstring(frag):
        ...
        if start:
            if html_lead_ignore(item):
                continue
            lead.append(etree.tostring(item, encoding="unicode"))
    if not lead:
        return frag
    return "\n".join(lead)
...
def html_strip_tags(frag):
    ...
    return etree.tostring(html, method="text", encoding="unicode")
...
def template_to_dict(tree):
    ...
    for item in tree:
        try:
            ...
            obj["%s ERROR" % __name__] = etree.tostring(item, encoding="unicode")
    return dict(obj)
